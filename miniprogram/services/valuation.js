...
function extractTonghuashunAsciiReportForecast(html, totalShares) {
  ...
  while ((yearMatch = yearPattern.exec(text))) {
    ...
    const countMatches = [...text.matchAll(/profit-forecast-tab-count-\d+">\s*(\d+)/g)].map((match) => Number(match[1])).filter((value) => Number.isFinite(value));
    const institutionCount = Math.max(sum(countMatches), 1);
    return {
      forecastItems: years.map((year, index) => ({ year, netProfit: profits[index] * unitMultiplier, eps: totalShares ? (profits[index] * unitMultiplier) / totalShares : null })),
      institutionCount,
...
module.exports = {
  ...
  _test: {
    ...
    getMarketCapAnchor,
    extractTonghuashunReportForecast,
    extractTonghuashunAsciiReportForecast
  }
};
